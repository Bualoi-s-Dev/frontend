...
const handleClick = () => {
  if (!props.isOwner) router.push(`/package/${props.packageId}/subpackage/schedule/${props.subpackageId}`);
}
...
<template>
  ...
  <div v-if="showConfirmDialog" class="fixed top-0 left-0 w-screen h-screen bg-black opacity-50 z-[101]"></div>
  ...
  <button @click="handleClick"
    ...
    <div class="flex justify-between items-center w-full">
      ...
      <div v-if="props.isOwner" class="flex gap-[10px]">
        <Icon icon="icon-park-outline:edit-two" class="w-[27px] h-[27px] text-body cursor-pointer" @click="editItem" />

        <Icon icon="pajamas:remove-all" class="w-[27px] h-[27px] text-body cursor-pointer" @click="confirmDelete" />
      </div>
    </div>
    ...
    <div class="flex py-[5px] gap-[10px]">
      <p v-for="(day, index) in days" :key="index"
        class="flex justify-center w-[36px] py-[5px] border rounded-[5px] text-[12px]" :class="{
  ...
  <div v-if="showConfirmDialog" class="fixed inset-0 flex items-center justify-center z-[102]">
    <div class="flex gap-[20px] bg-white p-[25px] rounded-lg shadow-lg w-[346px]">
      <Icon icon="pajamas:remove-all" class="w-[50px] h-[50px] text-body cursor-pointer" />
      <div class="flex flex-col gap-[20px]">
        <p class="text-[16px] text-titleActive">
          Are you sure you want to delete this subpackage?
        </p>
        <div class="flex justify-between">
          <button class="text-[14px] text-titleActive py-[8px] px-[30px] border border-titleActive rounded-[6px]"
