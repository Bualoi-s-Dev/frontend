<template>
  ...
  <div class="p-4">
    ...
    <p class="flex justify-center mt-5 text-xl">{{ formatDate(selectedDate) }}</p>
    <div>
      <ul class="flex flex-col gap-5 my-5">
        <li v-for="(interval, index) in intersectedIntervals" :key="index">
          <div :class="['rounded-2xl drop-shadow-xl p-7 text-white', interval.isIntersected ? 'bg-red-schedule' : 'bg-green-schedule']">
...
const getTimeIntervals = (startTime: string, endTime: string, duration: number) => {
  const intervals = [];
...
const packageDetails = {
  ...
  id: "12345678abcd",
  isInf: false,
  packageId: "12345678abcd",
  price: 10000,
  repeatedDay: ["MON", "TUE", "WED"],
...
const timesIntersect = (intervalStart: string, intervalEnd: string, eventStart: string, eventEnd: string): boolean => {
...
const intersectedIntervals = computed(() => {
  return intervals.map(interval => {
    ...
    const intersectingEvent = events.value.find(event =>
      ...
      timesIntersect(interval.start, interval.end, event.start, event.time.split(" - ")[1])
    );

    return { ...interval, isIntersected: intersectingEvent ? true : false };
  });
});


const events = ref<Event[]>([
  { id: 1, title: "Zürich design days", start: "9:00 AM", time: "9:00 AM - 10:00 AM", date: "2025-03-12", color: "border-green-500 bg-green-100" },
  { id: 2, title: "Prepare workshop", start: "11:00 AM", time: "11:00 AM - 12:00 PM", date: "2025-03-12", color: "border-blue-500 bg-blue-100" },
