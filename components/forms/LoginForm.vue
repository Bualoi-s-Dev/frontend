...
const handleSubmit = () => props.onSubmit(imageUrl.value, name.value, type.value);
...
<template>
  <div class="w-full h-full p-6 flex flex-col">
    ...
    <div class="flex flex-row text-lg mt-6">Name<span class="text-primary">*</span></div>
    <input v-model="name" type="text" class="border border-stroke w-full rounded-md py-1 pl-2 text-lg mt-1.5" />

    <div class="flex flex-row text-lg mt-6">Type<span class="text-primary">*</span></div>
    ...
    <button :disabled="!valid" @click="handleSubmit" class="mt-auto ml-auto text-lg px-6 py-2 rounded-lg bg-black text-white disabled:opacity-50">
      Submit
    </button>
  </div>
